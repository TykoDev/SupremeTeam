...
| A saved latest run is unreadable or partially corrupt | Preserve any readable artifacts and take the label the classifier actually returns — the two are different states with different causes. `corrupt` means a record was read and found incoherent (an unparseable pointer, a pointer naming a missing or stateless run, a broken revision lineage); `unreadable` means `skillset-saves/` exists but no run state could be read at all. Either way, attempt read-only resume from the earliest provable boundary and otherwise continue transiently with a clear warning, never overwriting the evidence — but report the label the classifier gave, because filing a `corrupt` run as `unreadable` discards the detail naming what was incoherent. |
