...
- Before starting a new run, classify `skillset-saves/` with `save_run.py status --run-id <id>` into one of the ten values of `../save-protocol.md` §2 Startup (active/inactive/complete/stale/orphaned/conflicting/corrupt/interrupted/missing/unreadable) and resume an active reclaimable run — `active` or `orphaned` — before creating new state.
