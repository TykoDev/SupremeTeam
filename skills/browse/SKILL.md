...
description: >-
  ...
  observable evidence instead of guessing selectors, and redacts credentials from
...
1. Establish the target surface, required browser state, and safe interaction boundary before opening or reusing a session. **Validate the target URL first**: require an `http` or `https` scheme and refuse internal or loopback targets (localhost, 127.0.0.0/8, 169.254.169.254, RFC-1918 ranges, `*.internal`) and non-web schemes (`file:`, `chrome:`, etc.) unless the user has explicitly authorized the internal target — this prevents SSRF-style misuse where a supplied URL probes the internal network. **Authorization covers network targets only.** A user can authorize an internal host, a loopback port, or an RFC-1918 address, and that is a normal thing to do when testing a local surface. Nothing authorizes a non-web scheme: `file:`, `chrome:`, `chrome-extension:`, `devtools:`, `view-source:`, and the rest are refused unconditionally, because they read the host's own filesystem and the browser's own internals rather than a page under test — which is not the thing this skill observes, whoever asks. When no session exists yet, acquire the browser through `open-browser`'s Browser Acquisition ladder — reuse an installed or cached browser before installing the Playwright browser (attaching to the user's running browser is opt-in).
...
| The supplied URL targets an internal or loopback address (localhost, RFC-1918, `169.254.169.254`, `*.internal`) | Refuse navigation, explain the restriction, and ask the user to confirm they intend to target an internal resource. Their explicit confirmation unlocks it; record that it was given. |
| The supplied URL uses a non-web scheme (`file:`, `chrome:`, `chrome-extension:`, `devtools:`, `view-source:`) | Refuse, and do not offer confirmation as a route through. These read the host filesystem or the browser's internals rather than a page under test, so no authorization makes them in scope for this skill. If the user needs a local file rendered, ask them to serve it over `http://localhost` and authorize that instead. |
| The walked flow downloads a file — an export, a report, a generated document | The download leaves the redaction pass entirely: it is written by the browser, not by a capture step, and step 4 never sees it. Treat it as unredacted content from an authenticated session. Record that the download occurred, its filename, and its size as the observation; do not open it to describe its contents, and never write it to the save path. Leave it in the browser's own download directory and name that location, or delete it once the flow is confirmed. If the flow's *evidence* is the file's content, that is a request for the user to make with the file in hand, not something a walkthrough exports on their behalf. |
