...
- **Vendoring detection**: classify every changed path as first-party or not *before* any completeness claim is made. A non-first-party path is inventoried, excluded from first-party coverage rather than counted toward it, and never hand-edited. `references/contracts.md` carries the mechanical classification rule and the inventory fields; this phase owns that rule, and `build/security-builder` cross-points to it so both treat the same path the same way.
