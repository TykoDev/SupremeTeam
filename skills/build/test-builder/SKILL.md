...
3. Discover the project's runner before running anything — the discovery ladder is in `references/workflow.md`. Take the runner from what discovery returns, never from habit: at the time of writing this repository resolves to `python -m unittest` and has no pytest installed, so a pytest command written from memory fails on invocation. If discovery later reports pytest, use pytest; the rule is that the ladder decides, not that unittest is permanent.
...
- **Coverage output belongs to the run**: Coverage data files and reports are run evidence, never project-root residue. Resolve the destination with `python skills/scripts/output_paths.py --kind coverage`, point the runner's data file and report output at it, and leave nothing named `.coverage`, `.coverage.*`, `.coverage/`, `htmlcov/`, or `.nyc_output/` at the project root when the step ends — one observed run left a `.coverage` tree of over three thousand files there in under two minutes. `references/workflow.md` § Coverage destination carries the per-runner flags, the exact resolver invocation, and the parallel-mode rule.
