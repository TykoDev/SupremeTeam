...
| REDESIGN | An existing user-facing surface needs a new design with alternatives to compare | redesign | Surface in scope, parity-defining flows, current source or captures, Taste snapshot | Design inventory, taste grilling log, four directions, four living variants, parity and rendered and accessibility evidence, comparison, recorded decision | redesign writes the redesign package; design-mapper the inventory and parity evidence; taste the grilling log; architect the directions; prototyper one variant per delegation |
