...
| Redesign parity | Map before changing anything: a stable-id inventory of the current design is the parity contract, and four differentiated living variants are compared on evidence before one is built. | partial: `redesign-review` requires `variant_set` and `parity_evidence`, produced with [`check_parity.py`](../scripts/check_parity.py) | [redesign](../design/redesign/SKILL.md) and [design-mapper](../design/design-mapper/SKILL.md) |
