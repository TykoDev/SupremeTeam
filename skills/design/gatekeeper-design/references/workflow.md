...
3. Run the boundary validator, `../../../harness/gatekeeper/check.py --boundary <boundary>`, against `../../../gates.yaml`. `design-to-build` requires `decisions`, `architecture`, `interfaces`, `plan`, `acceptance`, `security_seed`, `stack_lock`, `taste_snapshot`, and `ui_evidence`, with `decisions`, `architecture`, `plan`, and `taste_snapshot` backed by hashed artifacts. `redesign-review` requires `design_inventory`, `taste_grilling`, `taste_snapshot`, `design_directions`, `mock_set`, `mock_parity`, `mock_rendering`, `selection`, `selected_variant`, `parity_evidence`, `rendered_verification`, `accessibility_evidence`, `recommendation`, and `residual_risk`, with only the last three unbacked. Read `selection` before the four keys that depend on it. Read both key lists from the spec, never from memory; `boundary-evidence.md` carries the per-key detail.
4. Resolve every `UNCHECKED` finding explicitly. At `design-to-build` the conditional artifacts are the API contracts and the frontend/UI handoff; at `redesign-review` they are the capture keys: an `UNCHECKED` on `mock_rendering` is never a waiver, because that key is `no_fallback` at this boundary, and an `UNCHECKED` on `rendered_verification` is resolved against `selection.decision` — evidence when a variant was selected, the sanctioned wording when it was not.
5. Check the packet for alignment across problem framing, constraints, architecture, API contracts, implementation specification, and locked technology choices. At `redesign-review`, judge instead whether the four directions are genuinely differentiated, whether each of the four drafts stayed a mock rather than becoming an implementation built before the choice, whether the comparison is honest, whether the recommendation follows from it, and whether the recorded selection is a decision a person made.
...
- Any waiver is a typed applicability record for a sanctioned reason; `mock_rendering` at `redesign-review` accepts none, because that boundary lists it under `no_fallback`.
- At `redesign-review`: exactly four mocks with unique ids and hashed per-mock artifacts; one aggregated mock parity probe bound by `inputs` to the inventory; a well-formed `selection`; and, when its `decision` is `variant`, exactly one selected variant whose id equals `selection.chosen`, with its own full-level parity probe, captures, and accessibility findings. When the decision is a merge or a deferral, those four keys instead carry the matching sanctioned wording as the `reason` of an applicability record.
...
- Frontend/UI handoff schema: `../../../design-doctrine.md` §9 also governs direction differentiation at `redesign-review`, the boundary the SKILL.md contract does not name, along with what a mock may contain and the rule that implementation follows selection.
