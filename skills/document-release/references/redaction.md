...
| Internal URLs | The whole URL, **including the query string** — a link to an internal tool leaks the host, the path structure, and frequently ids, filters, tokens, and account references inside its parameters | Two different things, and the split is the point. **The external draft gets nothing**, or a public URL that genuinely serves the same purpose (a docs page, a status page) — never a description of an internal tool, which tells a reader something exists that they cannot reach and names it for them. **The internal follow-up record gets the description**, and the original URL, so the fact is preserved for the team. |
