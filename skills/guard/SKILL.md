...
- **`allow_dangerous` is a global kill-switch, not a scoped exception.** While it is lifted, every destructive pattern the hook knows passes for every command in the session — not only for the operation it was requested for. So it is never the default, never enabled silently, and revoked (`guard_state.py revoke-dangerous --requester <owner>`) the moment the operation completes rather than left to expire. An expired or malformed grant leaves the block in force; exactly one shape lifts it with no end — the legacy bare `true`, which the writer never produces. **Size `--minutes` to the whole operation plus margin**, because a grant that lapses mid-sequence denies the *next* step and leaves the work half-done. `references/enforcement.md` § allow_dangerous override protocol carries the four grant shapes, what the recorded confirmation does and does not prove, and what to do when a grant lapses mid-flight; read it before granting or trusting one.
...
| Two sessions record or release boundaries at the same time | Expect a lost write, and verify rather than assume. `guard_state.py` is read-modify-write with no lock: it loads the whole record, edits it in memory, and replaces the file atomically. The replace cannot corrupt the file, but a second writer that loaded *before* the first one saved will overwrite the first one's change — so a freeze recorded during another session's release can vanish, silently and with exit 0. Treat any concurrent guard work as unconfirmed until `guard_state.py status` shows the expected record, re-run the command if it does not, and where two people are working the same boundary, serialize on one of them rather than both calling the writer. |
