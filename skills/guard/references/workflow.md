...
3. Record the guard state through `python skills/harness/hooks/guard_state.py` (`block`, `freeze`, `read-only`) — the record's only writer — together with the blocked actions and any override rule needed to cross the boundary.
...
- Owner overrides must be verifiable, not implied by context or tone. An `allow_dangerous` grant is the widest of them: it lifts destructive-pattern blocking for the whole session, so it carries a named owner, a scope, a reason, and the shortest expiry the operation needs.
