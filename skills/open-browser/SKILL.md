...
| The Playwright browser cache is detected at rung 3 but cannot be used — the directory is locked by another process, a binary is truncated or partially downloaded, or permissions deny execution | Record the cache path, the exact error, and that rung 3 was reached but not usable, then fall through to rung 4 exactly as an incompatible system binary falls through to rung 3. Do not delete or repair the cache: it may be mid-download by another session, and removing it turns a recoverable fall-through into a re-download for everyone using that host. Rung 4's announce-and-escalate rules apply unchanged — an unusable cache is not consent to install. |
