...
- Session scope and expiry expectations are recorded, by reference — no credential value appears in the session record, logs, or captured screenshots.
...
- `browse` consumes a ready visible session to walk the interface.
- `pair-agent` consumes a known-good visible session when another operator needs access.
