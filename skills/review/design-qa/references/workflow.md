...
- `design/redesign` is the delegating owner at `redesign-review`, where this lens produces two keys rather than one: `mock_rendering` for the four mocks, and `rendered_verification` for the selected variant. It delegates the mock-review and visual-qa stages, sets the scope of each, and is the owner a `REVISE` on either key returns through. It also writes the sanctioned applicability record for `rendered_verification` when the selection was a merge or a deferral; this lens never writes that record and never declines a capture on the strength of one.
- `design/prototyper` builds the mocks and the selected variant this lens captures. A capture is bound by sha256 to that skill's `mock.html` / `app.html` and `tokens.css`, so a rebuilt file invalidates the record and the stage re-runs rather than re-labels.
