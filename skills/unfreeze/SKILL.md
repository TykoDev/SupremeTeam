...
| The live `allow_dangerous` is the legacy bare `true` rather than a grant object | Anyone can revoke it, and that is the safe direction — but say so in the record rather than implying an owner authorized the lift. `cmd_revoke_dangerous` guards its owner check with `isinstance(current, dict)`, so a bare `true` skips authority entirely: the revoke succeeds for any `--requester` and `released_by` names whoever happened to run it. The legacy shapes documented for globs apply to this key too, and this is the one where a legacy shape *loosens* rather than blocks. Revoke it, then have an owner re-record any grant that is still genuinely needed through `allow-dangerous`, so the replacement carries an owner, a scope, and an expiry. |
